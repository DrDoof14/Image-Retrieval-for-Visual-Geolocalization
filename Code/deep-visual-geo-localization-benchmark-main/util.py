...
def resume_model(args, model,second_model = None):
    if second_model:
        print('model2 is resumed from : ',args.resume2)
        checkpoint = torch.load(args.resume2, map_location=args.device)
      
    else :
         checkpoint = torch.load(args.resume, map_location=args.device)
         print('model1 is resumed from : ',args.resume)
