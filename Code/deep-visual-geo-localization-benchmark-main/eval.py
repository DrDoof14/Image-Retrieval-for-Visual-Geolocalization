...
if args.Ensembel ==True:
  ...
  model2 = network.GeoLocalizationNet(args,second_model=True)
  model2 = model2.to(args.device)
  
  # for param_tensor in model2.state_dict():
  #   print(param_tensor)

  model1 = util.resume_model(args, model1)
  
  model2 = util.resume_model(args, model2,second_model = True)
  ...
  if args.pca_dim is None:
      ...
      pca = util.compute_pca(args, model, args.pca_dataset_folder, full_features_dim)
  print('\n pca :    ',pca)
  recalls, recalls_str = test.test_ensm(args, test_ds, model1,model2, args.test_method, pca)
