...
def test_ensm(args, eval_ds, model1,model2, test_method="hard_resize", pca=None):
    """Compute features of the given dataset and compute the recalls."""
    ...
    with torch.no_grad():
        ...
        all_features = np.empty((len(eval_ds),16640), dtype="float32")
        tem_t =0
        ...
        for inputs, indices in tqdm(database_dataloader, ncols=100): 
            ...
            features1 = model1(inputs.to(args.device))
            features1 = features1.cpu().numpy()
            features2 = model2(inputs.to(args.device))
            features2 = features2.cpu().numpy()           
            if tem_t ==0:
              print('\n model 1 feature size is {} model2 feature size is{}' .format(features1.shape,features2.shape))
            ...
            if pca is not None:
                features = pca.transform(features)
            all_features[indices.numpy(), :] = np.concatenate((features1,features2),axis =1)
            if tem_t ==0: 
              print('\n  all feature size is:{}' .format(all_features.shape))
              tem_t =1
    ...
    faiss_index = faiss.IndexFlatL2(16640)
    faiss_index.add(database_features)
    del database_features, all_features
