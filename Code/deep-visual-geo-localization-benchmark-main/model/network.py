...
class GeoLocalizationNet(nn.Module):
    ...
    """
    def __init__(self, args,second_model =False):
        super().__init__()
        self.backbone = get_backbone(args,second_model)
        self.arch_name = args.backbone
        self.aggregation = get_aggregation(args,second_model )
 
        print('.aggregation2',args.aggregation2)
        print('second_model',second_model)
        if second_model:
          if args.aggregation2 in ["gem", "spoc", "mac", "rmac"]:
...
def get_aggregation(args,second_model ):

    if second_model:
          if args.aggregation2 == "gem":
              return aggregation.GeM(work_with_tokens=args.work_with_tokens)
          elif args.aggregation2 == "spoc":
              return aggregation.SPoC()
          elif args.aggregation2 == "mac":
              return aggregation.MAC()
          elif args.aggregation2 == "rmac":
              return aggregation.RMAC()
          elif args.aggregation2 == "netvlad":
...
def get_backbone(args,second_model):
    ...
    if args.backbone.startswith("resnet"):
        ...
        if args.backbone.startswith("cct384"):
            print('\n backbone.startswith("cct384") \n')
            if second_model:
               backbone = cct_14_7x2_384(pretrained=True, progress=True, aggregation=args.aggregation2)
            else:
            ...
            for name, child in backbone.encoder.layer.named_children():
                if int(name) > args.freeze_te:
                    for params in child.parameters():
                        params.requires_grad = True
        if second_model:
                  backbone = VitWrapper(backbone, args.aggregation2)
